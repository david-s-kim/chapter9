...
class MediaPlayerService : Service() {
    ...
    // foreground 를 사용하기때문에 onBind 는 구현안함
    ...
    // notification 채널을 만들어주기위한 사전작업업
    ...
    // mainActivity 내용도 함께 가져와야함, 제어도 같이하기때문
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when(intent?.action){
            MEDIA_PLAYER_PLAY -> {
                if(mediaPlayer == null) {
                    mediaPlayer = MediaPlayer.create(baseContext, R.raw.hiswill)
                }
                mediaPlayer?.start()
            }
            MEDIA_PLAYER_PAUSE -> {
